...
public class DefaultMessageTransfer implements MessageTransfer {
    ...
    @Override
    public void send(final ClusterRemotingCommand message) {
        ...
        for (final ServerNode node : activeNodes){
    ...
    @Override
    public void send(final String nodeName, final ClusterRemotingCommand message) {
        long timeoutMillis = 3000;
        final ServerNode node = ClusterNodeManager.getInstance().getNode(nodeName);
        String addr = node.getAddr();
        try {
            this.clusterRemotingClient.invokeAsync(addr, message, timeoutMillis, new InvokeCallback() {
                @Override
